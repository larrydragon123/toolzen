...
export const zh: TranslationDict = {
  ...
  tools: {
    ...
    'pdf-merge': {
      ...
      howToTitle: '如何使用PDF合并工具',
      howTo: '点击"添加文件"选择多个PDF文件（或直接拖入），通过拖拽调整文件排序，点击"合并PDF"按钮即可将多个PDF合并为一个文件并下载。所有处理在浏览器本地完成，文件不会上传到任何服务器。',
      faq: [
        { q: '合并后的PDF文件顺序是怎样的？', a: '按照列表中文件的排列顺序合并，你可以通过拖拽调整文件顺序。' },
        { q: '文件大小有限制吗？', a: '理论上没有限制，但受浏览器内存影响，建议单个PDF不超过100MB。合并多个大文件时请耐心等待。' },
        { q: '文件会上传到服务器吗？', a: '不会。ToolZen所有工具都在浏览器本地运行，你的文件始终保留在设备上。' },
      ],
    },
    'pdf-split': {
      title: 'PDF 拆分',
      ...
      howToTitle: '如何使用PDF拆分工具',
      howTo: '上传PDF文件后，可以看到每页的缩略图预览。选择拆分方式：按页码范围（如1-3,5,7-9）提取指定页面，或每N页拆分为一个独立的PDF。点击拆分后自动打包为ZIP下载。所有处理在浏览器本地完成。',
      faq: [
        { q: '拆分后的文件格式是什么？', a: '拆分后的每个部分都是独立的PDF文件，打包为ZIP格式方便下载。' },
        { q: '页码范围怎么填写？', a: '用逗号分隔，支持范围表示。例如"1-3,5,7-9"表示提取第1到3页、第5页、第7到9页。' },
        { q: '文件会上传吗？', a: '不会。所有拆分操作在浏览器本地完成。' },
      ],
    },
    'pdf-compress': {
      title: 'PDF 压缩',
      ...
      howToTitle: '如何使用PDF压缩工具',
      howTo: '上传PDF文件后自动显示原始文件大小。选择压缩等级（基础/标准/极限），点击压缩按钮，工具会显示压缩后的大小和压缩率，满意后点击下载。所有处理在浏览器本地完成。',
      faq: [
        { q: '压缩等级有什么区别？', a: '基础模式仅去除PDF冗余数据，不损失画质（约减少20-40%）。标准模式对图片进行适度压缩（减少40-60%）。极限模式最大程度压缩（减少60-80%），画质会有所降低。' },
        { q: '压缩会损失内容质量吗？', a: '基础模式完全无损。标准和极限模式主要压缩内嵌图片，文字内容不受影响。' },
        { q: '文件会上传吗？', a: '不会。所有压缩操作在浏览器本地完成，文件不会离开你的设备。' },
      ],
    },
  },
  about: {
  ...
  suggest: {
    title: '建议新工具',
    subtitle: '告诉我们你还想要什么工具，我们会尽快评估添加',
    toolName: '工具名称',
    toolNamePlaceholder: '例如：JSON 转 CSV 转换器',
    description: '功能描述',
    descriptionPlaceholder: '描述你希望这个工具有什么功能...',
    email: '邮箱（选填）',
    emailPlaceholder: '方便我们联系你跟进',
    submit: '提交建议',
    submitting: '提交中...',
    success: '收到！我们会尽快评估你的建议。',
    error: '提交失败，请稍后重试。',
  },
  relatedTools: '相关工具',
